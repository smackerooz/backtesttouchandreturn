...
# ------------------------------------------------------------
# Helper functions with retries and delays
# ------------------------------------------------------------
def download_with_retry(func, *args, max_retries=3, delay=1, **kwargs):
    """Generic retry wrapper for yfinance downloads."""
    for attempt in range(max_retries):
        try:
            ...
            if "Rate limit" in str(e) or "Too Many Requests" in str(e):
                ...
                st.warning(f"Rate limit hit. Waiting {wait:.1f} seconds...")
                time.sleep(wait)
            else:
                # Non-rate-limit error, just return None
                return None
    return None

@st.cache_data(ttl=3600)
def get_daily_data(ticker, start, end):
    """Download daily OHLC with retry and rate limit handling."""
...
def get_15min_data(ticker, start, end):
    """
    Download 15-minute intraday data (Yahoo only last 60 days).
    Clamps start date to (today - 60 days) automatically.
    """
    ...
    df = download_with_retry(
        yf.download,
        ticker, start=effective_start_dt, end=end, interval="15m", progress=False
    )
    if df is None or df.empty:
        return None
    if isinstance(df.columns, pd.MultiIndex):
...
def extract_setups(ticker, daily_df, intraday_df, atr_period, range_threshold):
    """Scan each day's 15-min bars to find valid trade setups."""
    ...
    for date, bars in grouped:
        ...
        stop_price = entry_price - 0.191 * candle_range
...
if run_btn:
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            # Wait 1 second between stocks to avoid rate limits
            time.sleep(1)
            intra = get_15min_data(ticker, start_date, end_date)
            if daily is None or intra is None:
                st.warning(f"⚠️ Skipping {ticker}: missing data.")
                continue
